...
- **Keep subagent waits short; put long waits in the main thread.** A subagent's prompt cache is short-lived, and a foreground wait that outlives it rewrites the subagent's whole context at full price. Keep each subagent wait short enough that its cache stays warm, and prefer a wake that fires when the work finishes over re-running a short wait; a wait that must run long belongs in the main thread, whose cache lasts far longer. Appending is the expensive act and re-reading is cheap, so a cheaper `model` override pays off on a short read (an Explore pass, a digest), not on a long-lived delegate. In a headless run, never yield the turn to background work that gates the next step; the session ends when you yield.
...
- **Get independent eyes on a risky diff.** Risky means a destructive path, a credential surface, a returned-object trust boundary, a contract or public interface, or a behavior change without tests; size alone is not risk, and a large mechanical change with CI and a bot reviewer relies on those instead. For a risky diff, the core's self-review shares the blind spots that wrote the code. Run `/code-review` yourself when it is available, or spawn a fresh-context reviewer subagent with the diff and the stated intent, prompted to refute rather than confirm, and focused on correctness, security, regressions, and missing tests. `/code-review ultra` is user-triggered and billed; suggest it rather than launching it.
